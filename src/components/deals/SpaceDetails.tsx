...
export function SpaceDetails({
	...
	const saved: Draft = {
		terms:
			space.marketing.spaceLeaseTerms?.[0] ?? emptySpaceLeaseTerms(unit.id),
	...
	return (
		// `gap-6 p-4` — the group tier, so the frame spaces the form's groups the
		// same way `ListingEditor` does rather than crowding them at 12px.
		<div className="d-flex flex-column gap-6 p-4">
			<ListingPageHeader title="Details" />

			{/* Availability is the space deal's stage — see spaceAvailability() — not
			    a field, so it reads as a note about the page rather than sitting
			    inside one of the groups as though it belonged to those fields. */}
			...
			{/* Cancel ghost, Save primary — the same bar the deal edit form ends
			    with, so the two forms behave identically. Disabled until something
			    changes, since a Save that does nothing teaches nothing. */}

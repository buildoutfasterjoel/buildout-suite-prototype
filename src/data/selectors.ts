...
export function getContactDetailClient(id: string): ContactDetail | null {
  ...
  const toView = (l: Listing) => (t: Listing['tasks'][number]): ContactTask => {
    ...
    return {
      ...
      createdByAi: t.createdByAi ?? false,
      dealId: l.id,
      dealName: l.name,
    }
  }
...
// ── All-tasks list (standalone Tasks page) ──────────────────────────────────

const TASK_ROSTER: Teammate[] = [CURRENT_USER, ...TEAMMATES]

/** Stable string hash → used to give deal tasks a deterministic real assignee. */
...
/**
 * Every task across the whole workspace — standalone {@link Task}s plus each
 * deal's embedded planner {@link DealTask}s — normalized into {@link TaskView}
 * for the Tasks page. Standalone tasks keep their real assignee/type; deal tasks
 * (which carry only placeholder initials and a free-text label) get a
 * deterministic real assignee and an inferred, scoped type.
 */
export function listAllTasks(): TaskView[] {
  const { tasks, listings, contacts } = useDataStore.getState()
  const memberById = new Map(TASK_ROSTER.map((m) => [m.id, m]))
  const out: TaskView[] = []

  // Standalone tasks (created via the Add Task modal).
  for (const t of tasks.values()) {
    const m = memberById.get(t.assigneeId)
    let sourceKind: TaskView['sourceKind'] = 'none'
    let sourceLabel = ''
    if (t.source === 'deal' && t.dealId) {
      sourceKind = 'deal'
      sourceLabel = listings.get(t.dealId)?.name ?? 'Deal'
    } else if (t.source === 'contact' && t.contactId) {
      const c = contacts.get(t.contactId)
      sourceKind = 'contact'
      sourceLabel = c ? `${c.firstName} ${c.lastName}`.trim() : 'Contact'
    }
    out.push({
      id: t.id,
      kind: 'standalone',
      ...
      assigneeName: m?.name ?? 'Unassigned',
      assigneeInitials: m?.initials ?? t.assigneeInitials,
      assigneeAvatarUrl: m?.avatarUrl,
      dueDate: t.dueDate,
  ...
  // Deal-embedded planner tasks.
  for (const l of listings.values()) {
    for (const t of l.tasks) {
      const m = TASK_ROSTER[hashString(t.id) % TASK_ROSTER.length]
      out.push({
        id: t.id,
        kind: 'deal',
        dealId: l.id,
        title: t.label,
        type: deriveTaskType(t.label),
        assigneeId: m.id,
        assigneeName: m.name,
        assigneeInitials: m.initials,
        assigneeAvatarUrl: m.avatarUrl,

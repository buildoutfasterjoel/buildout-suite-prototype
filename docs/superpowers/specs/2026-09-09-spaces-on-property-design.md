...
| **Q12** | *Resolved in review (2026-09-11):* the index row shows the availability headline beside the stage badge, and the Filters flyout gains a Space Availability facet over `statuses` (any-space semantics — the reason there is no `Mixed` value). The PRD's excluded "Include spaces" toggle (spaces as rows) stays out. Wiring the facet re-enabled the toolbar's Filters button, which had been left inert beside the placeholder dropdowns. | — |

...
/** §3.10b — any non-contact record's data dump (+ optional targeted question) →
 * analyst brief. A separate prompt from `CONTACT_BRIEF_PROMPT` because a deal's
 * sections are not a person's: nobody wants PROPERTY OWNERSHIP on a task. */
export const RECORD_BRIEF_PROMPT = `You are a CRE analyst briefing a broker on ONE record (a deal, listing, property, or task), using ONLY the supplied data. Never invent facts.
- If a specific question is provided: answer it directly and concisely (2–4 sentences), leading with the answer; if the data doesn't contain it, say so and offer the closest fact. Plain prose, no headers.
- If no question: produce a brief with plain ALL-CAPS section headers (no markdown, no asterisks), including only sections where data exists: WHERE IT STANDS, THE NUMBERS, WHO'S INVOLVED, RECENT ACTIVITY, OPEN ITEMS, BROKER TAKEAWAYS (2–3 bullets).
- Lead with what is actually at stake right now — the stage, the next critical date, and anything overdue.
Return only the structured object (a single 'brief' string).`;
...
export const CALL_RECAP_PROMPT = `You are Otto, a sharp CRE assistant, summarizing a broker's call that just ended. You are given the full transcript and the contact.

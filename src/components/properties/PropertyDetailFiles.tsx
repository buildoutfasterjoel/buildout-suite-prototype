...
export function PropertyDetailFiles({ listingId }: { listingId: string }) {
  ...
  const path = useMemo(
    () => breadcrumbPath(items, currentFolderId),
  ...
  const trashedRoots = useMemo(() => {
    const deletedIds = new Set(
      items.filter((i) => i.deletedAt).map((i) => i.id),
    );
    return items
  ...
  return (
    <div className="d-flex flex-column gap-3 p-4" style={{ minWidth: 0 }}>
      ...
      {/* Search — left aligned, on its own row */}
      ...
      {/* Always-visible upload dropzone */}
      <div
        role="button"
        tabIndex={0}
        ...
        onDrop={handleDrop}
        className={isDraggingOver ? "bg-primary-subtle rounded-3" : ""}
      >
        <Empty variant="inline" className="justify-content-center">
          <Empty.Media>
            <FontAwesomeIcon icon={faCloudArrowUp} aria-hidden />
          </Empty.Media>
          <Empty.Content className="flex-grow-0">
            Drop files here or click to upload
          </Empty.Content>
        </Empty>
      </div>

      {/* Breadcrumb — sits underneath the search bar */}
      <Breadcrumb>
        <Breadcrumb.List>
          <Breadcrumb.Item>
      ...
      <div>
        {sortedChildren.length === 0 ? (
          <Empty>
            ...
            <Empty.Content>
              <Empty.Title>
                {search.trim() ? "No files match your search" : "No files yet"}
              </Empty.Title>
            </Empty.Content>
          </Empty>
        ) : (
      ...
      {/* Recycle Bin — opens a modal listing trashed items */}
      <div>
        <Button
          variant="ghost"
          size="sm"
      ...
      <Dialog open={recycleBinOpen} onOpenChange={setRecycleBinOpen}>
        <Dialog.Content>
          <Dialog.Header>
            <Dialog.Title>Recycle Bin</Dialog.Title>
          </Dialog.Header>
          <Dialog.Body>
          ...
          <Dialog.Body>
            <Field>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setMoveDestination(v === "root" ? null : v)
                }
              >
                <Select.Trigger>

...
function projectInfoSection(c: Ctx, property: Property | undefined): Section {
  // The label, not the raw union member — see `projectInfo` in underwritingResult.
  const type = property ? TYPE_LABELS[property.propertyType] : "Multifamily";
...
function buildSummaryPage(property: Property | undefined, uw: DealUnderwriting, c: Ctx): Page {
  ...
  return {
    ...
    blocks: [
      ...
      // Named, not anonymous: the broker who ran it is the one who answers for
      // the numbers, and it's the same name the BOV's cover email signs off with.
      text(`Prepared by ${CURRENT_USER.name}, ${CURRENT_USER.role} · ${CURRENT_USER.email}`, subtitleStyle),
      text(
